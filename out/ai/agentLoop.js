...
const blockCatalog_1 = require("./blockCatalog");
...
${blockCatalog_1.CANVAS_BLOCK_CATALOG}`;
}
...
function safeChatMessage(raw, recovered) {
    if (recovered?.final?.message)
        return recovered.final.message;
    const body = stripFences(raw);
...
function historyToMessages(history) {
    ...
    for (const turn of history) {
        ...
        if (!text)
            continue;
        const content = text.length > MAX_HISTORY_CHARS ? `${text.slice(0, MAX_HISTORY_CHARS)}\n…(truncated)` : text;
...
async function runAgentLoop(args) {
    ...
    const label = phase === 'home'
        ? 'Home orchestrator'
        : args.label || phase;
    const userParts = [`USER: ${text}`, ''];
    if (phase === 'home') {
        userParts.push('CONTEXT: User is on the Home screen. To add docs to the pipeline call generate_pipeline. To draft one, finish with both "document" (BlockNote blocks) and "targetDoc" (id or exact name from list_pipeline / generate_pipeline). Never claim a canvas was filled without that.', '');
    }
    else if (fieldGuide) {
    ...
    for (let iter = 0; iter < MAX_ITERS; iter++) {
        const raw = await (0, llmClient_1.callLlm)(messages, llmConfig, { jsonMode: true });
        lastRaw = raw;
        ...
        if (step?.tool) {
            let observation = await (0, tools_1.runTool)(step.tool, step.args ?? {}, ctx);
